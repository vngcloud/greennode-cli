...
func init() {
	...
	listCmd.Flags().String("name", "", "Filter by DHCP option name (substring match)")
}
...
func runList(cmd *cobra.Command, args []string) error {
	...
	result, err := apiClient.Get(fmt.Sprintf("/v2/%s/dhcp_option", projectID), params)
	if err != nil {
		return fmt.Errorf("failed to list DHCP options: %w", err)
	}

	return outputDhcpList(cmd, cfg, result)
}
